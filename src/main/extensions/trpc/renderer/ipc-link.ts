...
import type { RendererGlobalElectronTRPC } from 'trpc-electron/src/types'
import { transformResult } from 'trpc-electron/src/renderer/utils'
...
const getElectronTRPC = () => {
  const electronTRPC: RendererGlobalElectronTRPC = (globalThis as any).electronExtensionTRPC

  if (!electronTRPC) {
    throw new Error(
      'Could not find `electronExtensionTRPC` global. Check that `exposeElectronTRPC` has been called in your preload file.'
    )
  }
